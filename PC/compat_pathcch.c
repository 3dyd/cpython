/*
 * Origin: https://github.com/adang1345/api-ms-win-core-path/
 * Modifications: prefixed API functions with '_'; removed DllMain; merged .c and .h
 *
 *
...
HRESULT WINAPI _PathAllocCanonicalize(const WCHAR* path_in, DWORD flags, WCHAR** path_out);
HRESULT WINAPI _PathAllocCombine(const WCHAR* path1, const WCHAR* path2, DWORD flags, WCHAR** out);
HRESULT WINAPI _PathCchAddBackslash(WCHAR* path, SIZE_T size);
HRESULT WINAPI _PathCchAddBackslashEx(WCHAR* path, SIZE_T size, WCHAR** end, SIZE_T* remaining);
HRESULT WINAPI _PathCchAddExtension(WCHAR* path, SIZE_T size, const WCHAR* extension);
HRESULT WINAPI _PathCchAppend(WCHAR* path1, SIZE_T size, const WCHAR* path2);
HRESULT WINAPI _PathCchAppendEx(WCHAR* path1, SIZE_T size, const WCHAR* path2, DWORD flags);
HRESULT WINAPI _PathCchCanonicalize(WCHAR* out, SIZE_T size, const WCHAR* in);
HRESULT WINAPI _PathCchCanonicalizeEx(WCHAR* out, SIZE_T size, const WCHAR* in, DWORD flags);
HRESULT WINAPI _PathCchCombine(WCHAR* out, SIZE_T size, const WCHAR* path1, const WCHAR* path2);
HRESULT WINAPI _PathCchCombineEx(WCHAR* out, SIZE_T size, const WCHAR* path1, const WCHAR* path2, DWORD flags);
HRESULT WINAPI _PathCchFindExtension(const WCHAR* path, SIZE_T size, const WCHAR** extension);
BOOL    WINAPI _PathCchIsRoot(const WCHAR* path);
HRESULT WINAPI _PathCchRemoveBackslash(WCHAR* path, SIZE_T path_size);
HRESULT WINAPI _PathCchRemoveBackslashEx(WCHAR* path, SIZE_T path_size, WCHAR** path_end, SIZE_T* free_size);
HRESULT WINAPI _PathCchRemoveExtension(WCHAR* path, SIZE_T size);
HRESULT WINAPI _PathCchRemoveFileSpec(WCHAR* path, SIZE_T size);
HRESULT WINAPI _PathCchRenameExtension(WCHAR* path, SIZE_T size, const WCHAR* extension);
HRESULT WINAPI _PathCchSkipRoot(const WCHAR* path, const WCHAR** root_end);
HRESULT WINAPI _PathCchStripPrefix(WCHAR* path, SIZE_T size);
HRESULT WINAPI _PathCchStripToRoot(WCHAR* path, SIZE_T size);
BOOL    WINAPI _PathIsUNCEx(const WCHAR* path, const WCHAR** server);
...
HRESULT WINAPI _PathAllocCanonicalize(const WCHAR* path_in, DWORD flags, WCHAR** path_out)
{
    ...
    if (root_end)
    {
        ...
        if (_PathCchStripPrefix(dst, length + 6) == S_OK)
        {
    ...
    *dst = 0;
    ...
    if (flags & PATHCCH_ENSURE_TRAILING_SLASH)
        _PathCchAddBackslash(buffer, buffer_size);
...
HRESULT WINAPI _PathAllocCombine(const WCHAR* path1, const WCHAR* path2, DWORD flags, WCHAR** out)
{
    ...
    if (!path1 || !path2) return _PathAllocCanonicalize(path1 ? path1 : path2, flags, out);
    ...
    _PathCchStripPrefix(combined_path, combined_length);
    if (add_backslash) _PathCchAddBackslashEx(combined_path, combined_length, NULL, NULL);

    if (path2 && path2[0])
    {
        if (path2[0] == '\\' && path2[1] != '\\')
        {
            _PathCchStripToRoot(combined_path, combined_length);
            path2++;
        }

        _PathCchAddBackslashEx(combined_path, combined_length, NULL, NULL);
        lstrcatW(combined_path, path2);
    }

    hr = _PathAllocCanonicalize(combined_path, flags, out);
...
HRESULT WINAPI _PathCchAddBackslash(WCHAR* path, SIZE_T size)
{
    return _PathCchAddBackslashEx(path, size, NULL, NULL);
}

HRESULT WINAPI _PathCchAddBackslashEx(WCHAR* path, SIZE_T size, WCHAR** endptr, SIZE_T* remaining)
{
...
HRESULT WINAPI _PathCchAddExtension(WCHAR* path, SIZE_T size, const WCHAR* extension)
{
    ...
    hr = _PathCchFindExtension(path, size, &existing_extension);
...
HRESULT WINAPI _PathCchAppend(WCHAR* path1, SIZE_T size, const WCHAR* path2)
{
    return _PathCchAppendEx(path1, size, path2, PATHCCH_NONE);
}
...
HRESULT WINAPI _PathCchAppendEx(WCHAR* path1, SIZE_T size, const WCHAR* path2, DWORD flags)
{
    ...
    hr = _PathCchCombineEx(result, size, path1, path2, flags);
...
HRESULT WINAPI _PathCchCanonicalize(WCHAR* out, SIZE_T size, const WCHAR* in)
{
    ...
    return _PathCchCanonicalizeEx(out, size, in, PATHCCH_NONE);
}
...
HRESULT WINAPI _PathCchCanonicalizeEx(WCHAR* out, SIZE_T size, const WCHAR* in, DWORD flags)
{
    ...
    hr = _PathAllocCanonicalize(in, flags, &buffer);
    if (FAILED(hr)) return hr;
...
HRESULT WINAPI _PathCchCombine(WCHAR* out, SIZE_T size, const WCHAR* path1, const WCHAR* path2)
{
    return _PathCchCombineEx(out, size, path1, path2, PATHCCH_NONE);
}
...
HRESULT WINAPI _PathCchCombineEx(WCHAR* out, SIZE_T size, const WCHAR* path1, const WCHAR* path2, DWORD flags)
{
    ...
    hr = _PathAllocCombine(path1, path2, flags, &buffer);
    if (FAILED(hr))
    {
        out[0] = 0;
        return hr;
...
HRESULT WINAPI _PathCchRemoveBackslash(WCHAR* path, SIZE_T path_size)
{
    ...
    return _PathCchRemoveBackslashEx(path, path_size, &path_end, &free_size);
}
...
HRESULT WINAPI _PathCchRemoveExtension(WCHAR* path, SIZE_T size)
{
    ...
    hr = _PathCchFindExtension(path, size, &extension);
    if (FAILED(hr)) return hr;
...
HRESULT WINAPI _PathCchRemoveFileSpec(WCHAR* path, SIZE_T size)
{
    ...
    if (FAILED(_PathCchSkipRoot(path, (const WCHAR**)&root_end)))
        root_end = path;
...
HRESULT WINAPI _PathCchRenameExtension(WCHAR* path, SIZE_T size, const WCHAR* extension)
{
    ...
    hr = _PathCchRemoveExtension(path, size);
    if (FAILED(hr)) return hr;

    hr = _PathCchAddExtension(path, size, extension);
...
HRESULT WINAPI _PathCchStripToRoot(WCHAR* path, SIZE_T size)
{
    ...
    if (_PathCchSkipRoot(path, &root_end) == S_OK)
    {
